...
package metapi.javamail.mail.smtp;
...
import metapi.javamail.mail.event.*;
import metapi.javamail.mail.internet.*;

import metapi.javamail.mail.util.*;
import metapi.javamail.mail.auth.*;

/**
 * This class implements the Transport abstract class using SMTP for
 * message submission and transport. <p>
 *
 * See the <a href="package-summary.html">com.sun.javamail.smtp</a> package
 ...
 *
 * @see metapi.javamail.mail.event.ConnectionEvent
 * @see metapi.javamail.mail.event.TransportEvent
 */
...
public class SMTPTransport extends Transport {
	...
	// setting javamail.smtp.quitwait to false causes us to not wait for the
	// response from the QUIT command
	quitWait = PropUtil.getBooleanSessionProperty(session,
				"javamail." + name + ".quitwait", true);

	// javamail.smtp.reportsuccess causes us to throw an exception on success
	reportSuccess = PropUtil.getBooleanSessionProperty(session,
				"javamail." + name + ".reportsuccess", false);

	// javamail.smtp.starttls.enable enables use of STARTTLS command
	useStartTLS = PropUtil.getBooleanSessionProperty(session,
				"javamail." + name + ".starttls.enable", false);

	// javamail.smtp.starttls.required requires use of STARTTLS command
	requireStartTLS = PropUtil.getBooleanSessionProperty(session,
				"javamail." + name + ".starttls.required", false);

	// javamail.smtp.userset causes us to use RSET instead of NOOP
	// for isConnected
	useRset = PropUtil.getBooleanSessionProperty(session,
				"javamail." + name + ".userset", false);

	// javamail.smtp.noop.strict requires 250 response to indicate success
	noopStrict = PropUtil.getBooleanSessionProperty(session,
				"javamail." + name + ".noop.strict", true);
     ...
     * The property javamail.smtp.localhost overrides javamail.smtp.localaddress,
	...
	if (localHostName == null || localHostName.length() <= 0)
	    localHostName =
		    session.getProperty("javamail." + name + ".localhost");
	if (localHostName == null || localHostName.length() <= 0)
	    localHostName =
		    session.getProperty("javamail." + name + ".localaddress");
	try {
	    if (localHostName == null || localHostName.length() <= 0) {
	...
	if (saslMechanisms == UNKNOWN_SA) {
	    ...
	    String s = session.getProperty("javamail." + name + ".sasl.mechanisms");
     ...
     * an {@link metapi.javamail.mail.smtp.SMTPAddressSucceededException
     * SMTPAddressSucceededException} will be included in the exception
     * chain for each successful address, along with the usual
     * {@link metapi.javamail.mail.smtp.SMTPAddressFailedException
     ...
     *
     * Unless javamail.smtp.ehlo is set to false, we'll try to identify
     * ourselves using the ESMTP command EHLO.
     *
     * If javamail.smtp.auth is set to true, we insist on having a username
	...
	// setting javamail.smtp.ehlo to false disables attempts to use EHLO
	boolean useEhlo =  PropUtil.getBooleanSessionProperty(session,
					"javamail." + name + ".ehlo", true);
	// setting javamail.smtp.auth to true enables attempts to use AUTH
	boolean useAuth = PropUtil.getBooleanSessionProperty(session,
					"javamail." + name + ".auth", false);
	...
	/*
	 * If javamail.smtp.auth is set, make sure we have a valid username
	...
	// setting javamail.smtp.auth.mechanisms controls which mechanisms will
	// be used, and in what order they'll be considered.  only the first
	// match is used.
	String mechs = session.getProperty("javamail." + name + ".auth.mechanisms");
	if (mechs == null)
	...
	while (st.hasMoreTokens()) {
	    String m = st.nextToken();
	    String dprop = "javamail." + name + ".auth." +
	...
	if (saslAuthenticator == null) {
		...
		Class<SaslAuthenticator> sac = (Class<SaslAuthenticator>) Class.forName(
                "com.sun.javamail.smtp.SMTPSaslAuthenticator");
     ...
     * and the <code>javamail.stmp.sendpartial</code> property is not set,
     ...
     *
     * If some of the <code>addresses</code> fail the SMTP check,
     * and the <code>javamail.stmp.sendpartial</code> property is set to true,
	...
	if (!use8bit)
	    use8bit = PropUtil.getBooleanSessionProperty(session,
				"javamail." + name + ".allow8bitmime", false);
	if (logger != null && logger.isLoggable(Level.FINE)) {
	...
	try {
	    ...
	    // so if javamail.smtp.userset is set we use RSET instead of NOOP.
	    if (useRset)
		sendCommand("RSET");
	    else
		sendCommand("NOOP");
	     ...
	     * Argh!
	     *
	     * If javamail.smtp.noop.strict is set to false, be tolerant of
     ...
     * <li>javamail.smtp.from property</li>
	...
	if (from == null || from.length() <= 0)
	    from = session.getProperty("javamail." + name + ".from");
	if (from == null || from.length() <= 0) {
	    Address[] fa;
	    Address me;
	...
	if (supportsExtension("DSN")) {
	    ...
	    if (ret == null)
		ret = session.getProperty("javamail." + name + ".dsn.ret");
	...
	if (supportsExtension("AUTH")) {
	    ...
	    if (submitter == null)
		submitter = session.getProperty("javamail." + name + ".submitter");
	    // XXX - check for legal syntax?
	    if (submitter != null) {
		try {
	...
	if (ext == null)
	    ext = session.getProperty("javamail." + name + ".mailextension");
	...
	if (!sendPartial)
	    sendPartial = PropUtil.getBooleanSessionProperty(session,
					"javamail." + name + ".sendpartial", false);
	if (sendPartial)
	...
	if (supportsExtension("DSN")) {
	    ...
	    if (notify == null)
		notify = session.getProperty("javamail." + name + ".dsn.notify");
	    // XXX - check for legal syntax?
	    if (notify != null)
		dsn = true;
	}
	...
	if (sendFailed) {
		...
		validUnsentAddr[i++] = valid.elementAt(j);
	    for (int j = 0; j < validUnsent.size(); j++)
		validUnsentAddr[i++] = validUnsent.elementAt(j);
	...
	try {
	    serverSocket = SocketFetcher.startTLS(serverSocket, host,
				session.getProperties(), "javamail." + name);
	    initStreams();
	} catch (IOException ioex) {
	    closeConnection();
	...
	return  traceLogger != null && traceLogger.isLoggable(Level.FINEST);
    }
	...
	if (traceLogger != null && traceLogger.isLoggable(Level.FINEST)) {
