...
package metapi.javamail.mail.internet;

import metapi.javamail.mail.MessagingException;
import metapi.javamail.mail.EncodingAware;
 ...
 *
 * RFC 822 javamail headers <strong>must</strong> contain only US-ASCII
 ...
 * that contains only ASCII characters is already javamail-safe. If the
 ...
 * <p>
 * Note that to get the actual bytes of a javamail-safe String (say,
 ...
 *
 * The <code>javamail.mime.decodetext.strict</code> property controls
 ...
 * If the <code>javamail.mime.decodetext.strict</code> System property is
 * set to <code>"false"</code>, an attempt will be made to decode these
 * illegal encoded words. The default is true. <p>
 *
 * The <code>javamail.mime.encodeeol.strict</code> property controls the
 ...
 * without change.  If the <code>javamail.mime.encodeeol.strict</code>
 * System property is set to <code>"true"</code>, such an encoding will
 * be used when necessary.  The default is false. <p>
 *
 * In addition, the <code>javamail.mime.charset</code> System property can
 ...
 * javamail messages is different than the charset used for files stored on
 * the system, this property should be set. <p>
 *
 * The current implementation also supports the following System property.
 * <p>
 * The <code>javamail.mime.ignoreunknownencoding</code> property controls
...
public class MimeUtility {
	...
	PropUtil.getBooleanSystemProperty("javamail.mime.decodetext.strict", true);
    private static final boolean encodeEolStrict =
	PropUtil.getBooleanSystemProperty("javamail.mime.encodeeol.strict", false);
    private static final boolean ignoreUnknownEncoding =
	PropUtil.getBooleanSystemProperty(
	    "javamail.mime.ignoreunknownencoding", false);
    /*
	...
	PropUtil.getBooleanSystemProperty("javamail.mime.foldencodedwords", false);
    private static final boolean foldText =
	PropUtil.getBooleanSystemProperty("javamail.mime.foldtext", true);


    /**
     * Get the Content-Transfer-Encoding that should be applied
     * to the input stream of this DataSource, to make it javamail-safe. <p>
     *
     ...
     * <code>javamail.mime.ignoreunknownencoding</code> system property is set to
    ...
    /**
     * Encode a RFC 822 "text" token into javamail-safe form as per
     * RFC 2047. <p>
     *
    ...
    /**
     * Encode a RFC 822 "word" token into javamail-safe form as per
     * RFC 2047. <p>
     *
     * The given Unicode string is examined for non US-ASCII
     * characters. If the string contains only US-ASCII characters,
     ...
     * @see	metapi.javamail.mail.internet.HeaderTokenizer#MIME
     * @see	metapi.javamail.mail.internet.HeaderTokenizer#RFC822
     */
     ...
     * default locale.  If the System property <code>javamail.mime.charset</code>
	...
	if (defaultJavaCharset == null) {
	    /*
	     * If javamail.mime.charset is set, it controls the default
	     * Java charset as well.
	     */
	    String mimecs = null;
	    try {
		mimecs = System.getProperty("javamail.mime.charset");
	...
	if (defaultMIMECharset == null) {
	    try {
		defaultMIMECharset = System.getProperty("javamail.mime.charset");
	    } catch (SecurityException ex) { }	// ignore it
	}
	if (defaultMIMECharset == null)
	...
	try {
	    ...
	    InputStream is = 
		    metapi.javamail.mail.internet.MimeUtility.class.getResourceAsStream(
