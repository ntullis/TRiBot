...
public class InternetAddress extends Address implements Cloneable {
	...
	PropUtil.getBooleanSystemProperty(
			    "javamail.mime.address.ignorebogusgroupname", true);
     ...
     * The entire email address may be specified in the "javamail.from"
     * property.  If not set, the "javamail.user" and "javamail.host" properties
	...
	if (session == null) {
	    ...
	    address = session.getProperty("javamail.from");
	    if (address == null) {
		user = session.getProperty("javamail.user");
		if (user == null || user.length() == 0)
		    user = session.getProperty("user.name");
		if (user == null || user.length() == 0)
		    user = System.getProperty("user.name");
		host = session.getProperty("javamail.host");
     ...
     * javamail addresses entered by a human.  Strict parsing is typically
     * used when parsing address headers in javamail messages.
     *
     * @param	addresslist	comma separated address strings
     * @param	strict		enforce RFC822 syntax
     * @return			array of InternetAddress objects
	...
	for (start = end = -1, index = 0; index < length; index++) {
		...
		if (parseHdr && !strict) {
		    ...
		    // or bogus javamail header name included in the address field
     ...
     * list addresses supported by most javamail servers.  Group addresses
