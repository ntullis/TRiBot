...
package metapi.javamail.mail.util.logging;

import metapi.javamail.mail.smtp.SMTPTransport;
import java.io.*;
...
import metapi.javamail.mail.internet.*;
import metapi.javamail.mail.util.ByteArrayDataSource;
 ...
 *      props.put("javamail.smtp.host", "my-javamail-server");
 *      props.put("javamail.to", "me@example.com");
 ...
 * javamail host for outgoing email.  The code to setup this handler via the
 * logging properties can be as simple as the following:
 *
 * <tt><pre>
 *      #Default MailHandler settings.
 *      com.sun.javamail.util.logging.MailHandler.javamail.smtp.host = my-javamail-server
 *      com.sun.javamail.util.logging.MailHandler.javamail.to = me@example.com
 *      com.sun.javamail.util.logging.MailHandler.level = WARNING
 *      com.sun.javamail.util.logging.MailHandler.verify = local
 * </pre></tt>
 *
 * For a custom handler, e.g. com.foo.MyHandler, the properties would be:
 * <tt><pre>
 *      #Subclass com.foo.MyHandler settings.
 *      com.foo.MyHandler.javamail.smtp.host = my-javamail-server
 *      com.foo.MyHandler.javamail.to = me@example.com
 *      com.foo.MyHandler.level = WARNING
 *      com.foo.MyHandler.verify = local
 * </pre></tt>
 *
 * All javamail properties documented in the <tt>Java Mail API</tt> cascade to the
 * LogManager by prefixing a key using the fully qualified class name of this
 * <tt>MailHandler</tt> or the fully qualified derived class name dot javamail
 * property.  If the prefixed property is not found, then the javamail property
 ...
 * {@linkplain metapi.javamail.mail.Authenticator} class used to provide login credentials
 ...
 * {@linkplain metapi.javamail.mail.internet.MimeUtility#getDefaultJavaCharset() default}
 * platform encoding).
 *
 * <li>&lt;handler-name&gt;.errorManager name of an
 * <tt>ErrorManager</tt> class used to handle any configuration or javamail
 ...
 *
 * <li>&lt;handler-name&gt;.javamail.bcc a comma separated list of
 ...
 *
 * <li>&lt;handler-name&gt;.javamail.cc a comma separated list of
 ...
 *
 * <li>&lt;handler-name&gt;.javamail.from a comma separated list of
 ...
 * (defaults to the {@linkplain metapi.javamail.mail.Message#setFrom() local address})
 *
 * <li>&lt;handler-name&gt;.javamail.host the host name or IP
 ...
 *
 * <li>&lt;handler-name&gt;.javamail.reply.to a comma separated
 ...
 * (defaults to <tt>null</tt>, none)
 *
 * <li>&lt;handler-name&gt;.javamail.to a comma separated list of
 ...
 * {@linkplain metapi.javamail.mail.internet.InternetAddress#getLocalAddress
 * local address}.)
 *
 * <li>&lt;handler-name&gt;.javamail.sender a single address
 ...
 * a {@linkplain metapi.javamail.mail.internet.MimeMessage#writeTo raw}
 ...
 * store, {@linkplain metapi.javamail.mail.internet.MimeMessage#MimeMessage(
 * metapi.javamail.mail.Session, java.io.InputStream) reconstruct}, and resend the
...
public class MailHandler extends Handler {
    ...
    /**
     * Creates a javamail handler with the given javamail properties.
    ...
    /**
     * Sets the javamail properties used for the session.  The key/value pairs
    ...
    /**
     * Gets a copy of the javamail properties used for the session.
    ...
    private String contentWithEncoding(String type, String encoding) {
        ...
        try {
            ...
            encoding = ct.toString(); //See javax.javamail.internet.ContentType.
            if (!isEmpty(encoding)) {
                type = encoding;
            }
     ...
     * @param props the given javamail properties.  Maybe null and are never
     ...
     * This triggers the LogManagerProperties to copy all of the javamail
        ...
        try {
            ...
            try {
                ...
                if (any != null && any.length != 0) {
                    ...
                    session.getProperty("javamail.transport.protocol"); //Force copy
                } else {
            ...
            if ("remote".equals(verify)) {
                ...
                try {
                    try {
                        ...
                        //to capture all javamail properties set in the LogManager.
                ...
                session.getProperty("javamail.host");
                session.getProperty("javamail.user");
                session.getProperty("javamail." + protocol + ".host");
                session.getProperty("javamail." + protocol + ".port");
                session.getProperty("javamail." + protocol + ".user");
                local = session.getProperty("javamail." + protocol + ".localhost");
                if (isEmpty(local)) {
                    local = session.getProperty("javamail."
     ...
     * javamail properties or authenticator.
     ...
     * provides the safe snapshot of the javamail properties.
    ...
    private void envelopeFor(Message msg, boolean priority) {
        setAcceptLang(msg);
        setFrom(msg);
        if (!setRecipient(msg, "javamail.to", Message.RecipientType.TO)) {
            setDefaultRecipient(msg, Message.RecipientType.TO);
        }
        setRecipient(msg, "javamail.cc", Message.RecipientType.CC);
        setRecipient(msg, "javamail.bcc", Message.RecipientType.BCC);
        setReplyTo(msg);
        setSender(msg);
        setMailer(msg);
    ...
    private void setMailer(final Message msg) {
        try {
            final Class<MailHandler> mail = MailHandler.class;
    ...
    private void setFrom(final Message msg) {
        final String from = msg.getSession().getProperty("javamail.from");
        if (from != null) {
            try {
    ...
    private void setReplyTo(final Message msg) {
        final String reply = msg.getSession().getProperty("javamail.reply.to");
        if (!isEmpty(reply)) {
            try {
    ...
    private void setSender(final Message msg) {
        ...
        final String sender = msg.getSession().getProperty("javamail.sender");
        if (!isEmpty(sender)) {
            try {
