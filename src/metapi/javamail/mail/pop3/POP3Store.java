...
package metapi.javamail.mail.pop3;
...
import metapi.javamail.mail.util.SocketConnectException;
import metapi.javamail.mail.util.MailConnectException;

/**
 * A POP3 Message Store.  Contains only one folder, "INBOX".
 *
 * See the <a href="package-summary.html">com.sun.javamail.pop3</a> package
...
public class POP3Store extends Store {
    ...
    volatile Constructor<POP3Message> messageConstructor = null;
	...
	String dir = session.getProperty("javamail." + name + ".filecache.dir");
	if (dir != null && logger.isLoggable(Level.CONFIG))
	    logger.config("javamail." + name + ".filecache.dir: " + dir);
	if (dir != null)
	    fileCacheDir = new File(dir);
	keepMessageContent = getBoolProp("keepmessagecontent");

	// javamail.pop3.starttls.enable enables use of STLS command
	useStartTLS = getBoolProp("starttls.enable");

	// javamail.pop3.starttls.required requires use of STLS command
	requireStartTLS = getBoolProp("starttls.required");

	String s = session.getProperty("javamail." + name + ".message.class");
	if (s != null) {
		...
		// now load the class
		Class<POP3Message> messageClass = null;
		try {
		    // First try the "application's" class loader.
		    // This should eventually be replaced by
		    // Thread.currentThread().getContextClassLoader().
		    messageClass = (Class<POP3Message>) Class.forName(s, false, cl);
		    ...
		    messageClass = (Class<POP3Message>) Class.forName(s);
		}

		Class[] c = {metapi.javamail.mail.Folder.class, int.class};
	...
	// if port is not specified, set it to value of javamail.pop3.port
	...
	p = new Protocol(host, portNum, logger,
	    session.getProperties(), "javamail." + name, isSSL);
