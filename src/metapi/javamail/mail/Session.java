...
import metapi.javamail.mail.util.LineInputStream;
import metapi.javamail.mail.util.MailLogger;

/**
 * The Session class represents a javamail session and is not subclassed.
 * It collects together properties and defaults used by the javamail API's.
 ...
 * are part of the JavaMail <code>javamail.jar</code> file. <p>
 *
 ...
 * file contents:
 * <pre>
 * protocol=imap; type=store; class=com.sun.javamail.imap.IMAPStore; vendor=Oracle;
 * protocol=smtp; type=transport; class=com.sun.javamail.smtp.SMTPTransport; vendor=Oracle;
 * </pre><p>
 *
 ...
 * </code>javax.javamail.Address</code> returns the address type.  The
 ...
 * <code>javax.javamail.Address</code> implementation that is present if it is
 * to be used.  For example, the
 * <code>javax.javamail.internet.InternetAddress</code> method
...
public final class Session {
    ...
    private final Hashtable<URLName, PasswordAuthentication> authTable = new Hashtable<URLName, PasswordAuthentication>();
    private boolean debug = false;
    private PrintStream out;			// debug output stream
    private MailLogger logger;
    private final Vector<Provider> providers = new Vector<Provider>();
    private final Hashtable<String, Provider> providersByProtocol = new Hashtable<String, Provider>();
    private final Hashtable<String, Provider> providersByClassName = new Hashtable<String, Provider>();
	...
	if (Boolean.valueOf(props.getProperty("javamail.debug")).booleanValue())
	    debug = true;

	initLogger();
	//logger.log(Level.CONFIG, "JavaMail version {0}", 1.4);
     ...
     *                  JavaMail spec (particularly  javamail.store.protocol,
     *                  javamail.transport.protocol, javamail.host, javamail.user,
     *                  and javamail.from) as the defaults are unlikely to
     *                  work in all cases.
     * @param	authenticator Authenticator object used to call back to
     *			the application when a user name and password is
     *			needed.
     * @return		a new Session object
     * @see	metapi.javamail.mail.Authenticator
     */
     ...
     * constructor, set the property <code>javamail.debug</code> in the
     * Properties object passed in to the constructor to true.  The
     * value of the <code>javamail.debug</code> property is used to
     * initialize the per-Session debugging flag.  Subsequent calls to
     * the <code>setDebug</code> method manipulate the per-Session
     * debugging flag and have no affect on the <code>javamail.debug</code>
     * property.
     *
     ...
     * as a result of setting the <code>javamail.debug</code> system property,
     ...
     * specified. Checks javamail.&lt;protocol&gt;.class property
	...
	// check if the javamail.<protocol>.class property exists
	String _className = props.getProperty("javamail."+protocol+".class");
	if (_className != null) {
	    if (logger.isLoggable(Level.FINE)) {
		logger.fine("javamail."+protocol+
	...
	props.put("javamail." + provider.getProtocol() + ".class",
     ...
     * protocol. The <code>javamail.store.protocol</code> property specifies the
	...
	return getStore(getProperty("javamail.store.protocol"));
    }
     ...
     * @see		metapi.javamail.mail.URLName
     * @exception	NoSuchProviderException If a provider for the given
     ...
     * Providers of IP-based javamail Stores should implement that
     ...
     * Transport protcol. The <code>javamail.transport.protocol</code> property
    ...
    public Transport getTransport() throws NoSuchProviderException {
        return getTransport(getProperty("javamail.transport.protocol"));
    }
     ...
     * @param	address
     * @return	A Transport object
     * @see metapi.javamail.mail.Address
	...
	transportProtocol =
	    getProperty("javamail.transport.protocol." + address.getType());
	...
	try {
	    Class[] c = {metapi.javamail.mail.Session.class, metapi.javamail.mail.URLName.class};
     ...
     * <p> <pre>
     * Connecting to &lt;protocol&gt; javamail service on host &lt;addr&gt;, port &lt;port&gt;.
     * &lt;prompt&gt;
     *
	...
	// load default META-INF/javamail.default.providers from javamail.jar file
	...
	if (providers.size() == 0) {
	    ...
	    addProvider(new Provider(Provider.Type.STORE,
			"imap", "com.sun.javamail.imap.IMAPStore",
			"Oracle", 1.4));
	    addProvider(new Provider(Provider.Type.STORE,
			"imaps", "com.sun.javamail.imap.IMAPSSLStore",
			"Oracle", 1.4));
	    addProvider(new Provider(Provider.Type.STORE,
			"pop3", "com.sun.javamail.pop3.POP3Store",
			"Oracle", 1.4));
	    addProvider(new Provider(Provider.Type.STORE,
			"pop3s", "com.sun.javamail.pop3.POP3SSLStore",
			"Oracle", 1.4));
	    addProvider(new Provider(Provider.Type.TRANSPORT,
			"smtp", "com.sun.javamail.smtp.SMTPTransport",
			"Oracle",1.4));
	    addProvider(new Provider(Provider.Type.TRANSPORT,
			"smtps", "com.sun.javamail.smtp.SMTPSSLTransport",
			"Oracle", 1.4));
	}
	...
	// load default META-INF/javamail.default.address.map from javamail.jar
