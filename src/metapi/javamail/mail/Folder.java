...
import metapi.javamail.mail.search.SearchTerm;
import metapi.javamail.mail.event.*;

/**
 * Folder is an abstract class that represents a folder for javamail
...
public abstract class Folder {
     ...
     * @see 		#HOLDS_FOLDERS
     * @see		#HOLDS_MESSAGES
     * @see		metapi.javamail.mail.event.FolderEvent
     */
     ...
     * a lightweight "check for new javamail" operation on a Folder without
     * opening it.  (For example, a thread that monitors a mailbox and
     * flags when it has new javamail.)  This method should indicate whether
     * any messages in the Folder have the <code>RECENT</code> flag set. <p>
     *
     * Note that this is not an incremental check for new javamail, i.e.,
     ...
     * @see		metapi.javamail.mail.event.MessageChangedEvent
     */
     ...
     * @see		metapi.javamail.mail.event.MessageCountEvent
     */
     ...
     * @exception       metapi.javamail.mail.search.SearchException if the search
     *			term is too complex for the implementation to handle.
     * @exception	FolderNotFoundException if this folder does 
     *			not exist.
     * @exception	IllegalStateException if this folder is not opened.
     * @exception       MessagingException
     * @see		metapi.javamail.mail.search.SearchTerm
     */
    ...
    private volatile Vector<EventListener> connectionListeners = null;
    ...
    addConnectionListener(ConnectionListener l) { 
   	if (connectionListeners == null) 
	    connectionListeners = new Vector<EventListener>();
    ...
    private volatile Vector<EventListener> folderListeners = null;
    ...
    public synchronized void addFolderListener(FolderListener l) { 
   	if (folderListeners == null)
	    folderListeners = new Vector<EventListener>();
    ...
    private volatile Vector<EventListener> messageCountListeners = null;
    ...
    public synchronized void addMessageCountListener(MessageCountListener l) { 
   	if (messageCountListeners == null)
	    messageCountListeners = new Vector<EventListener>();
    ...
    private volatile Vector<EventListener> messageChangedListeners = null;
    ...
    public synchronized void
   	...
   	if (messageChangedListeners == null)
	    messageChangedListeners = new Vector<EventListener>();
     ...
     */
    private void queueEvent(MailEvent event, Vector<EventListener> vector) {
